...
k = 7 # количество кубиков. На моем ноуте, при данных условиях, считает 4.34 сек. (с подключенным питанием 2.6 сек)
start = time.time()
lgr = 0    #int((n ** k - 1) / (n - 1)) к нулей
rgr = n ** k     # к граней с числом n-1 (т.к. нумеруем грани от 0 до n - 1)
li = []          # список, в котором будут храниться подсписки, соответствующие выпавшим комбинациям (количество выпавших значений на верхних гранях кубиков)
for i in range(lgr, rgr):     # 000...00 (к нулей) до (n-1)(n-1)...(n-1) тоже к цифр, после перевода в n-ричную систему счисления в строках 23-25
    r = ''
    ...
    # print(i2, dop)
    if dop not in li:          # если такой комбинации еще не было, добавляем в li.
        li.append(dop)
print(len(li))
